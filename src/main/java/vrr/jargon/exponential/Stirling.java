...
*/
    private static final double[] ERROR_HALVES = {
    ...
    public static double logError(final double n) {
        double nn;

        if (n <= 15.0) {
            nn = n + n;
            if (nn == (int)nn) return(ERROR_HALVES[(int)nn]);
            return(Gamma.logGamma(n + 1.) - (n + 0.5)*Math.log(n) + n - MathConstants.LN_SQRT_2PI);
        }
